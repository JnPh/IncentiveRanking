	...
	InitListeners: function()
		{
    		...
    		HomePage.joueurInfo.id = $(this).attr('data-id');
     		...
     		if(window.styleMedia.matchMedium("screen and (max-width: 480px)")){
    			...
    			else PlayerDetailsPage.LoadTabletJoueurData();       
			});
			...
			if(window.styleMedia.matchMedium("screen and (max-width: 480px)")){
     		...
     		else EventDetailsPage.LoadTabletEventData();
...
var EventDetailsPage =
{
	InitListeners: function()
		{
			$(document).on('pagebeforeshow', '#event', EventDetailsPage.FillinEventDetails); 
		},
    ...
    FillinEventDetails: function()
		{
		 $('#event-data').empty();
		    $.each(HomePage.eventInfo.result, function(i, row) {
		        if(row.data.ev_id == HomePage.eventInfo.id) {
		            ...
		            EventDetailsPage.GetParticipants(HomePage.eventInfo.id);          
		        }
		    });    
		},
...
var PlayerDetailsPage =
{
	InitListeners: function()
		{
			$(document).on('pagebeforeshow', '#headline', PlayerDetailsPage.FillinPlayerDetails); 
		},
		
	FillinPlayerDetails: function()
		{
			$('#joueur-data').empty();
		    $.each(HomePage.joueurInfo.result, function(i, row) {
		        if(row.data.jou_id == HomePage.joueurInfo.id) {
